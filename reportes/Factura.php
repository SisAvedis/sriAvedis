...
function addSociete( $nom, $ext_nom, $adresse,$logo,$ext_logo)
{
	...
	$this->Image($logo , 4 ,$y1-2, 25 , 25 , $ext_logo);
	$this->SetXY( $x1, $y1 );
	$this->SetFont('Arial','B',12);
	$length = $this->GetStringWidth( $nom );
	// $this->Cell( $length, 2, $nom);
	$this->Image($nom , 31 ,$y1-4, 31 , 10 , $ext_nom);
	$this->SetXY( $x1, $y1 + 7 );
...
function addDate( $date )
{
	...
	$this->Cell(10,5, utf8_decode("Fecha de Impresión"), 0, 0, "C");
...
// Operador address
function addCabecera($fecha, $areaAfectada, $notificante, $areaNotificante, $involucrados, $areaInvolucrada ){
	$r1     = $this->w - 192;
	$r2     = $r1 -7;
	$y1     = 38;
	$cantidadInvolucrados = count(explode(', ',$involucrados));
	...
	$this->SetFont( "Arial", "B", 10);
	$this->SetXY( $r1, $y1);
	$this->Line($r2, $y1, $r2, $y1+37);
	$this->Line($r2+190, $y1, $r2+190, $y1+37);
	$this->SetXY( $r1-5, $y1);
	$this->MultiCell( 190, 10,  utf8_decode("ÁREA AFECTADA: ".$areaAfectada));
	$this->SetXY( $r1+(10*12.5), $y1);
	$this->MultiCell( 190, 10,  utf8_decode("FECHA INCIDENTE: ".$fecha));
	$this->SetXY( $r2, $y1+(10*1));
	$this->MultiCell( 190, 0, '', 'B');
	$this->SetXY( $r1-5, $y1+(10*1));
	$this->MultiCell( 150, 10, "NOTIFICADO POR: ".$notificante);
	$this->SetXY( $r1+(10*12.5), $y1+(10*1));
	$this->MultiCell( 150, 10, utf8_decode("ÁREA: ".$areaNotificante));
	$this->SetXY( $r2, $y1+(10*2));
	$this->MultiCell( 190, 0, '', 'B');
	$this->SetXY( $r1-5, $y1+(10*2));
	$this->MultiCell( 150, 10, "INVOLUCRADOS: ");
	if($cantidadInvolucrados >6){
	$this->SetXY( $r1+27, $y1+(10*2.1));}
	else{
		$this->SetXY( $r1+27, $y1+(10*2.3));}
	$this->SetFont( "Arial", "", 9);
	$this->MultiCell( 95, 4, $involucrados);
	$this->SetXY( $r1+(10*12.5), $y1+(10*2.3));
	$this->SetFont( "Arial", "B", 10);
	$this->MultiCell( 50, 4, utf8_decode("ÁREA: ".$areaInvolucrada));
	$this->SetXY( $r2, $y1+(10*3));
	// $this->MultiCell( 190, 0, '', 'B');
...
function addFirmaSupervisor( $supervisor, $estado)
{
	if($estado === 'A' ){ 
	$r1     = $this->w - 180;
	$r2     = $r1 + 70;
	...
	$this->SetXY( $r1, $y1-3);
	$this->MultiCell( 40, 4, '', 'T');
	$this->SetXY( $r1, $y1);
	$this->SetFont( "Arial", "B", 10);
	$this->MultiCell( 40, 4, "Firma Supervisor ", 0 , "C");
	$this->SetXY( $r1-5, $y1+7);
	$this->SetFont( "Arial", "", 10);
	$this->MultiCell( 50, 4, $supervisor, 0 , "C");
}
function addFirmaInvolucrados( $involucrados, $estado)
{
	$splitInvolucrados = explode(', ',$involucrados);
	if ($estado == 'A'){
	$r1     = $this->w - 130;
	$r2     = $r1 + 120;
	$y1     = 174;
	...
	$this->SetXY( $r1+15, $y1);
	$this->SetFont( "Arial", "B", 10);
	$this->MultiCell( 40, 3, "Firma Involucrados" , 0 , "C");
	
	$this->SetFont( "Arial", "", 10);
	for($i=0;$i<count($splitInvolucrados);$i++){
		if($i < 6){
		$this->SetXY( ($r1+16), $y1+(16.2*($i+1)));
		$this->MultiCell( 40, 4, '', 'T');
		$this->SetXY( ($r1+11), $y1+(16.28*($i+1)));
	$this->Cell( 50, 4.2, $splitInvolucrados[$i], 0 , 0, "C");
		}
	if($i > 5){
		$this->SetXY( $r1+70, $y1+(16.2*($i-5)));
		$this->MultiCell( 40, 4, '', 'T');
		$this->SetXY( $r1+65, $y1+(16.28*($i-5)));
	$this->MultiCell( 50, 4.2, $splitInvolucrados[$i], 0 , "C");
	}
	}}
	else if ($estado == 'P'){
		$r1     = $this->w - 180;
	...
	$this->SetFont( "Arial", "", 10);
		for($i=0;$i<count($splitInvolucrados);$i++){
			if($i < 4){
			// $this->SetXY( ($r1+16), $y1+(16.2*($i+1)));
			$this->SetXY( $r1+(35*($i+1)),($y1+16) );
			$this->MultiCell( 32, 4, '', 'T');
			// $this->SetXY( ($r1+11), $y1+(16.28*($i+1)));
			$this->SetXY($r1+(35*($i+1)), ($y1+20) );
		$this->Text( $r1+(35*($i+1))+1, ($y1+20), $splitInvolucrados[$i]);
			}
			else if($i > 3 && $i < 8){
			// $this->SetXY( ($r1+16), $y1+(16.2*($i+1)));
			$this->SetXY( $r1+(35*($i-3)),($y1+36) );
			$this->MultiCell( 32, 4, '', 'T');
			// $this->SetXY( ($r1+11), $y1+(16.28*($i+1)));
			$this->SetXY($r1+(35*($i-3)), ($y1+40) );
		$this->Text( $r1+(35*($i-3))+1, ($y1+40), $splitInvolucrados[$i]);
			}
			else if($i > 7 && $i < 12){
			// $this->SetXY( ($r1+16), $y1+(16.2*($i+1)));
			$this->SetXY( $r1+(35*($i-7)),($y1+40) );
			$this->Line($r1+(35*($i-7)), $y1+55.5, $r1+(35*($i-7))+32, $y1+55.5);
			// $this->MultiCell( 32, 1, '', 'B');
			// $this->SetXY( ($r1+11), $y1+(16.28*($i+1)));
			$this->SetXY($r1+(35*($i-7)), ($y1+60) );
		$this->Text( $r1+(35*($i-7))+1, ($y1+60), $splitInvolucrados[$i]);
			}
		}	
	}

}
function addFirmaResponsables( )
{
	$r1     = $this->w - 125;
	$r2     = $r1 + 120;
	$y1     = 50;
	...
	$this->SetXY( $r1+40, $y1);
	$this->SetFont( "Arial", "B", 10);
	$this->MultiCell( 40, 4, "Firma Responsable" , 0 , "C");
	$this->SetXY( $r1+40, $y1+7);
	$this->SetFont( "Arial", "", 10);
	$this->MultiCell( 40, 4, utf8_decode("Martín Perez"), 0 , "C");
	// $this->SetXY( $r1+55, $y1-3);
	// $this->MultiCell( 40, 4, '', 'T');
	// $this->SetXY( $r1+55, $y1);
	// $this->SetFont( "Arial", "B", 10);
	// $this->MultiCell( 40, 4, "Firma Jefe de Planta" , 0 , "C");
	// $this->SetXY( $r1+55, $y1+7);
	// $this->SetFont( "Arial", "", 10);
	// $this->MultiCell( 40, 4, "Maximiliano Alcaraz", 0 , "C");
...
function addLine( $ligne, $tab, $border )
{
	...
	while ( list( $lib, $pos ) = each ($colonnes) )
	{
		...
		$this->MultiCell( $longCell, 4 , $texte."\n ", $border, $formText);
...
function addArchivosAdjuntos($stringArchivos){
	$r1  = $this->w - 134;
	$rf  = $this->w - 29;
	$y1  = $this->h - 18;
	$r2  = $this->w - 20 ;
	$y2  = $this->h - 235;
	if($stringArchivos === ''){
		$stringArchivos = 'Ninguno';
	}
	$this->Rect( $r1-65, $y1-4, $r2, $y2-50, "D");
	$this->SetFont('Arial','B',10);
	// $this->SetXY( $r1-50, $y1-3 );
	$this->Text($r1-62,$y1+3, 'ARCHIVOS ADJUNTOS: ');
	// $this->SetXY( $r1-18, $y1-2 );
	$this->SetFont('Arial','',10);
	$this->Text($r1-16,$y1+3, $stringArchivos);
}
